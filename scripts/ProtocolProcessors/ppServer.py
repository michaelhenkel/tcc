...
def createVirtualMachineInterface(tenant, vnName, sVid, cvlan=None):
    ...
    if bd:
        ...
        p_vmIntObj.add_bridge_domain(bridge_domain,bmem)
        p_vmIntObj.set_virtual_machine_interface_disable_policy(True)
        ...
        try:
            s_vmIntObjResult = vnc_client.virtual_machine_interface_create(s_vmIntObj)
        except:
            print 'cannot create vmi'
        vmIntObj = vnc_client.virtual_machine_interface_read(id = p_vmIntObjResult)
        s_vmIntObj = vnc_client.virtual_machine_interface_read(id = s_vmIntObjResult)
    else:
        instIpUUID = str(uuid.uuid4())
